...
(function($) {
	...
	// Carousels.
		...
		$(document).ready(function() {
			$('a[href^="#"]').scrolly({
				speed: 1500, // Adjust speed as necessary
		...
		document.getElementById('converter-form').addEventListener('submit', function(event) {
			...
			fetch('/convert', {
				...
				body: `from_currency=${from_currency}&to_currency=${to_currency}&amount=${amount}`
			})
			...
			.then(data => {
				if (data.result === 'success') {
					document.getElementById('result').innerHTML = `
						...
						<p>Converted Amount: ${data.converted_amount.toFixed(2)}</p>
					`;
				} else {
